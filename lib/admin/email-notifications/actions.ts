...
export async function queueDueBookingRemindersAction(): Promise<EmailNotificationsActionResult> {
  ...
  const supabase = createAdminClient();
  let result: { queued: number; skipped: number };

  try {
    result = await queueDueBookingReminders(supabase);
  } catch {
    return {
      status: "error",
  ...
  return {
    status: "success",
    message: `Queued ${result.queued} due reminder${result.queued === 1 ? "" : "s"}. Skipped ${result.skipped}.`,
  };
}
