...
- App guards require ALL of: an `auth.users` row whose `raw_app_meta_data.provider = 'azure'`, a matching `auth.identities` row (provider `azure`) whose `identity_data.tid` equals `public.microsoft_access_config.tenant_id`, an active `public.approved_users` allowlist row for the email, and a `public.profiles` row (created by the `on_auth_user_created` trigger). The seed sets all of these up.
